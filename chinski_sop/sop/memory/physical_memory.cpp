...
void sop::memory::PhysicalMemory::swap(SwapFile* file_swap,LogicalMemory* page_table,int8_t victim,int8_t frame)
{
  ...
  //zawartosc_ramki_do_swapa przerzucic(assigned_frames_queue.front()
   page_table->setPage(file_swap->getFreeFrame(),0,victim);//numer ramki w tablicy stron ustawiony na numer ramki ze swapa valid na 0
...
void sop::memory::PhysicalMemory::getFreeFrames(uint8_t pages_needed,sop::memory::LogicalMemory* table_of_pages,uint16_t pid,SwapFile* file_swap)
{
  ...
  while(isEnoughFrames(pages_needed)!=true)
  {
   ...
   //     victim_page=i;
   // } 
    swap(file_swap,table_of_pages,victim_page,this->assigned_frames_queue.front());//przerzucenie zawartosci struktury ramka do swapa
