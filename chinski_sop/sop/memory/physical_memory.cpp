...
uint8_t sop::memory::PhysicalMemory::getFreeFrames(uint8_t pages_needed,sop::memory::LogicalMemory* table_of_pages,uint16_t pid,SwapFile* file_swap)
{
  ...
  while(isEnoughFrames(pages_needed)!=true)
  {
    ...
    if(file_swap->getIsThereAnyFrameValue()==true)//sprawdzenie czy swap nie jest zapchany
    {
      ...
      this->list_of_free_frames.push_back(this->assigned_frames_deque.front());//wrzucenie ramki, ktorej strona zosta³a wymieciona na liste wolnych ramek
